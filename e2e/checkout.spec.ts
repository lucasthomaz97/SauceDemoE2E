...
test('Should return to products page when clicking continue shopping button on checkout page', async ({ loginPage, productsPage, checkoutPage }) => {
    await loginPage.login('standard_user', 'secret_sauce');
    await productsPage.shoppingCartLink.click();
...
test('Should display Your Information page after clicking checkout button on checkout page', async ({ loginPage, productsPage, checkoutPage }) => {
    await loginPage.login('standard_user', 'secret_sauce');
    await productsPage.getProductAddToCartButton(0).click();
    await productsPage.shoppingCartLink.click();
...
test('Should remove product from checkout page when clicking on remove', async ({ loginPage, productsPage, checkoutPage }) => {
    await loginPage.login('standard_user', 'secret_sauce');
    await productsPage.getProductAddToCartButton(0).click();
    await productsPage.shoppingCartLink.click();
...
test('Should keep products in cart after navigating back to products page from checkout page', async ({ loginPage, productsPage, checkoutPage }) => {
    await loginPage.login('standard_user', 'secret_sauce');
...
for (let i = 0; i < 6; i++) {
    test(`Should display information for product ${i + 1} in checkout page`, async ({ loginPage, productsPage, checkoutPage }) => {
        await loginPage.login('standard_user', 'secret_sauce');
    ...
    test(`Shoud display all the ${i + 1} products in checkout page after adding them to cart`, async ({ loginPage, productsPage, checkoutPage }) => {
        await loginPage.login('standard_user', 'secret_sauce');
        for (let j = 0; j <= i; j++) {
    ...
    test(`Should display all the ${i + 1} products in overview page after adding them to cart and filling Your Information form`, async ({ loginPage, productsPage, checkoutPage }) => {
        ...
        await checkoutPage.checkoutButton.click();
        await checkoutPage.expectYourInformationPage();
        await checkoutPage.yourInformationFirstNameInput.fill('Test');
        await checkoutPage.yourInformationLastNameInput.fill('User');
        await checkoutPage.yourInformationPostalCodeInput.fill('12345');
        await checkoutPage.yourInformationContinueButton.click();
        await checkoutPage.expectOverviewPage(i + 1);
    });
};

test('Should display Your Information page with empty inputs when clicking checkout button on checkout page', async ({ loginPage, productsPage, checkoutPage }) => {
    await loginPage.login('standard_user', 'secret_sauce');
    await productsPage.getProductAddToCartButton(0).click();
    await productsPage.shoppingCartLink.click();
    await checkoutPage.checkoutButton.click();
...
test('Should return to checkout page when clicking cancel button on Your Information page', async ({ loginPage, productsPage, checkoutPage }) => {
    await loginPage.login('standard_user', 'secret_sauce');
    await productsPage.getProductAddToCartButton(0).click();
    await productsPage.shoppingCartLink.click();
    await checkoutPage.checkoutButton.click();
...
test('Should display error message when trying to continue with empty First Name on Your Information page', async ({ loginPage, productsPage, checkoutPage }) => {
    await loginPage.login('standard_user', 'secret_sauce');
    await productsPage.getProductAddToCartButton(0).click();
    await productsPage.shoppingCartLink.click();
    await checkoutPage.checkoutButton.click();
